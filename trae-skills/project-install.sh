#!/usr/bin/env bash
# gstack Trae project install — Install skills to a specific project directory
# Usage: ./project-install.sh /path/to/project
# Result: Skills installed to /path/to/project/.trae/skills/
...
while [ $# -gt 0 ]; do
  case "$1" in
    --help|-h)
      ...
      echo ""
      echo "Install gstack skills to a specific project directory."
      echo "Skills will be installed to <target-directory>/.trae/skills/"
      echo ""
      echo "Arguments:"
      echo "  target-directory    The project directory to install skills to"
      echo ""
      echo "Options:"
      echo "  --help, -h          Show this help message"
      echo ""
      echo "Examples:"
      echo "  $0 /Users/boommanpro/my-project"
      echo "  # Installs to /Users/boommanpro/my-project/.trae/skills/"
      echo ""
      echo "  $0 ."
      echo "  # Installs to ./.trae/skills/ (current directory)"
      echo ""
      exit 0
      ;;
    -*)
    ...
    *)
      if [ -z "$TARGET_DIR" ]; then
        ...
        error "Multiple target directories specified. Use --help for usage information."
      fi
      shift
      ;;
  esac
...
if [ -z "$TARGET_DIR" ]; then
  error "No target directory specified. Use: $0 <target-directory>\nExample: $0 /path/to/your/project"
fi
...
create_trae_project_install() {
  info "Creating project Trae skills installation..."
...
print_instructions() {
  echo ""
  echo "=========================================="
  echo "  gstack for Trae IDE - Project Installation Complete!"
  echo "=========================================="
  echo ""
  ...
  echo "     $TRAE_SKILLS"
  echo ""
  echo "Or set environment variables in your project:"
  ...
  echo "  cd $GSTACK_ROOT && git pull && ./trae-skills/project-install.sh $TARGET_DIR"
  echo ""
}

main() {
  info "Starting gstack Trae project installation..."
